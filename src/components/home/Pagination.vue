...
<template>
  <Pagination v-slot="{ page }" v-model:page="model" :total="100" :sibling-count="1" show-edges :default-page="1" :items-per-page="15" class="flex justify-center py-4" @update:page="handleClick">
    <PaginationList v-slot="{ items }" class="flex items-center gap-1">
      ...
      <template v-for="(item, index) in items">
        <PaginationListItem v-if="item.type === 'page'" :key="index" :value="item.value" as-child>
          <Button class="lg:w-10 lg:h-10 lg:p-0 w-3.5 h-3.5 p-3.5" :variant="item.value === page ? 'default' : 'outline'">
            {{ item.value }}
          </Button>
        </PaginationListItem>
